...
export default {
    ...
    methods: {
        ...
        loadProject: function () {
            ...
            //TODO: studyId nije 1?
            const mldId =
                self.$route.params.studyId.substring(
                    0,
                    self.$route.params.studyId.length - 4
                ) + ".mld";

            const stdId = self.$route.params.studyId;
            var imgWidth, imgHeight;
            axios
                .get(constants.API_CORE_HOST + "/api/mld/dimensions/1/1/" + stdId + "/")
                .then((response) => {
                    ...
                    var baseLayer = new TileLayer({
                        ...
                        source: new XYZ({
                            url:
                            constants.API_CORE_HOST + "/api/region/" + stdId + "/{z}/{x}/{y}",
                            wrapX: false,
                        }),
                    });
                                                    ...
                                                    for (var j = 0; j < coordinates.length; j++) {
                                                        ...
                                                        if (coordinates2) {
                                                            for (var k = 0; k < coordinates2.length; k++) {
                                                                var x =
                                                                    res.features[i].geometry.coordinates[j][k][0] *
                                                                    1000 +
                                                                    imgWidth / 2;
                                                                x =
                                                                    (x * 40052752.78) / maxExtention - 20026376.39;
                                                                var y =
                                                                    imgHeight / 2 -
                                                                    res.features[i].geometry.coordinates[j][k][1] *
                                                                    1000;
                                                                y =
                                                                    ((maxExtention - y) * 40097932.2) /
                                                                    maxExtention -
                                                                    20048966.1;
                                                                res.features[i].geometry.coordinates[j][k][0] = x;
                                                                res.features[i].geometry.coordinates[j][k][1] = y;
                                                            }
                                                        }
                                                    }
                                                } else {
                    ...
                    var sourceAnnotations = new VectorSource({
                        ...
                        loader: function () {
                            var url =
                                constants.API_CORE_HOST + "/api/annotations/1/1/" + stdId + "/";
                            axios.get(url).then((response) => {
                                if (response.status == 200) {
                                    var res = response.data;
